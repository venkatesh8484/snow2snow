...
--
-- PURPOSE
-- This statement is a single `INSERT … SELECT` that builds a before/after ("old
-- vs. new") flight-leg comparison table. It reads two staging sources —
...
-- joins them on the flight-leg identity keys (airline code, flight number,
-- suffix code, scheduled departure date, departure station, arrival station)
-- using a **FULL OUTER JOIN**, and writes 60 columns into the target
...
-- (both sides present). This is the classic add/edit/update ("A/E/U") change-
-- data pattern produced by a full-outer-join diff.
--
-- ---
--
-- OPEN SME QUESTIONS
...
--
-- 1. **SEM-04 — timezone intent (pos 42):** Is the `TO_TIMESTAMP_LTZ` slot
--    intended to store *local* time, with `new_gmt_sched_departure_time` (pos 44)
--    as its GMT sibling? Should the session `TIMEZONE` be pinned (e.g. `ALTER
--    SESSION SET TIMEZONE = '…'`) before this INSERT, or should the expression use
...
--    concatenation use explicit `TO_CHAR(TO_DATE(o.sched_departure_date), 'YYYY-MM-
--    DD')` and an explicit time-to-char cast instead of relying on implicit
--    date/time→string casts? Does the `FF9` (nanosecond) parse mask match the
--    actual precision of `n.sched_departure_time`, or should it be `FF6`/`FF3`/
--    seconds?
--
-- 3. **SEM-10 — join-key grain (join ON):** Are `n.act_departure_station` /
--    `n.act_arrival_station` the same grain as `o.departure_station` /
--    `o.arrival_station` (i.e. scheduled station on both sides, despite the `act_`
--    naming on the new side)? If they are different grains (actual vs. scheduled),
--    the join is wrong and must be re-keyed.
--
-- 4. **SEM-08 — empty string vs NULL (pos 60):** Can `operating_airline_cd` ever
--    be an empty string `''` rather than `NULL` on either source? If yes, the `IS
--    NULL` probe in the `change_ind` CASE will not fire for those rows and the
...
SELECT
      ...
      -- SEM ▸ SEM-04 ▸ TO_TIMESTAMP_LTZ introduces a session-TIMEZONE dependency; confirm this slot is local time (GMT sibling at pos 44) and whether to pin session TIMEZONE or use TIMESTAMP_NTZ/TZ explicitly.
      TO_TIMESTAMP_LTZ(
          -- SEM ▸ SEM-06 ▸ CONCAT relies on implicit date/time→string casts; prefer explicit TO_CHAR and confirm the FF9 mask matches source sched_departure_time precision.
          CONCAT(TO_DATE(o.sched_departure_date), ' ', n.sched_departure_time),
          'YYYY-MM-DD HH24:MI:SS.FF9'
      -- SEM ▸ DEF-06 ▸ alias normalized from new_sched_departure_date to match the INSERT target at slot 42 (positional binding; expression unchanged).
      ...
      -- SEM ▸ SEM-08 ▸ change_ind CASE probes operating_airline_cd IS NULL; confirm operating_airline_cd cannot be '' (empty string is not NULL in Snowflake and would change the branch).
      CASE
          -- SEM ▸ A = leg exists only on the new side (add).
          WHEN o.operating_airline_cd IS NULL THEN 'A'
          -- SEM ▸ E = leg exists only on the old side (expire/remove).
          WHEN n.operating_airline_cd IS NULL THEN 'E'
          -- SEM ▸ U = leg exists on both sides (update).
          ELSE 'U'
      END AS change_ind
FROM   staging.wrk_synthetic_flgt_source1 o
-- SEM ▸ SEM-10 ▸ FULL OUTER JOIN over six identity keys; act_* station names on the new side vs bare station names on the old side — confirm same grain (scheduled vs actual).
FULL OUTER JOIN staging.wrk_synthetic_flgt_source2 n
       ...
       -- SEM ▸ SEM-03 ▸ if this or any join key is CHAR(n), Teradata blank-pads and ignores trailing spaces; Snowflake does not — TRIM/RTRIM may be required.
