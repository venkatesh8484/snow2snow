...
-- operational, and codeshare. Each branch picks the **latest** leg attributes
-- per flight (by `effective_dt`) using the "latest-value trick"
-- (`SUBSTR(MAX(effective_dt || <value>), 11, N)`), filters to legs whose
-- departure station differs from the arrival station, and only loads flights
-- newer than an `UPDATE_CUTOFF` parameter. Because the target is a SET table,
-- each branch must deduplicate **against the target's existing rows** (not
-- against the other branches); the fixed SQL therefore emits three independent
...
--
-- 1. **SEM-05 (sub) — target column order.** Is the assumed `airline_cd,
--    flgt_no, flgt_dt, company_cd, direction_ind, operating_airline_cd,
--    operating_flgt_no, operating_sfx_cd` order the actual physical column order of
--    `staging.wrk_target`? If not, the explicit `INSERT` column list and the
--    `EXCEPT SELECT *` positional match must be reordered to match the target DDL.
--    **Decision needed:** confirm or correct the column order against
--    `staging.wrk_target` DDL.
...
--    `CHAR(n)` in source or target? If so, Teradata blank-pads and ignores trailing
--    spaces in `=`, while Snowflake does not, so join/`EXCEPT` matching can drift.
--    **Decision needed:** confirm key column types; if `CHAR(n)`, wrap keys in
--    `TRIM`/`RTRIM`.
...
-- 6. **SEM-10 — join-key direction.** Are the marketing / operational /
--    codeshare join keys (`marketing_airline_cd`/`marketing_flgt_no`,
