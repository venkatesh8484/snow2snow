...
-- [FNX-01] line 164: ZEROIFNULL(best_usg.grs_rev_vlu) -> COALESCE(best_usg.grs_rev_vlu, 0)
-- [FNX-01] line 165: ZEROIFNULL(best_usg.grs_rev_curr_vlu) -> COALESCE(best_usg.grs_rev_curr_vlu, 0)
-- [FNX-03] line 304: LAST_DAY(dtrp.remittance_received_to_dt) - 3 -> DATEADD('day', -3, LAST_DAY(dtrp.remittance_received_to_dt))
-- [FNX-03] line 335: LAST_DAY(dtrp.remittance_received_to_dt) - 3 -> DATEADD('day', -3, LAST_DAY(dtrp.remittance_received_to_dt))
-- [SFX-01] line 451: up_ctry_grp.bus_efast_pd CONTAINS best_usg.up_dt -> best_usg.up_dt BETWEEN up_ctry_grp.bus_efast_start_dt AND up_ctry_grp.bus_efast_end_dt  (TODO(SME): confirm split column names)
-- [SFX-01] line 456: disch_ctry_grp.bus_efast_pd CONTAINS best_usg.up_dt -> best_usg.up_dt BETWEEN disch_ctry_grp.bus_efast_start_dt AND disch_ctry_grp.bus_efast_end_dt  (TODO(SME): confirm split column names)
-- [SFX-03] lines 409, 418, 478: QUALIFY ROW_NUMBER() OVER (...) = 1 kept as-is (valid in Snowflake)
-- [DEF-01] lines 80-81: duplicate `sac_cd` removed from INSERT column list (kept one)
-- [DEF-01] lines 236-237: duplicate `COALESCE(best_usg.sac_cd, MY_src.sac_cd) AS sac_cd` removed from SELECT (kept one)
-- [DEF-02] line 177: ASallow_piece_qty -> AS allow_piece_qty
-- [DEF-02] line 178: ASallow_weight_qty -> AS allow_weight_qty
-- [DEF-02] line 179: ASallow_weight_unit_cd -> AS allow_weight_unit_cd
-- [DEF-04] lines 212, 221: CAST((best_sa.dc_no, 0, 9999999999) AS BIGINT) -> CAST(COALESCE(best_sa.dc_no, 0) AS BIGINT)  (TODO(SME): confirm constant 0 / dropped 9999999999)
-- [DEF-04] lines 213, 222: CAST((best_sa.pme_dc_no, 0, 9999999999) AS BIGINT) -> CAST(COALESCE(best_sa.pme_dc_no, 0) AS BIGINT)  (TODO(SME): confirm constant 0 / dropped 9999999999)
-- [DEF-05] line 94 / (no SELECT expr): synthesized parallel CASE for yq_curr_vlu mirroring yq_vlu branches but returning original-currency values  (TODO(SME): confirm business rule)
-- [SEM-01] lines 270, 286, 305, 307, 315, 344: division yq_curr_vlu / ex_rte kept as `/` (decimal/decimal, no truncation risk per lessons learned)
-- [SEM-05] line 5: TODO(SME) — is wrk_dc_priority a SET table? If so, INSERT must be ported as INSERT INTO tgt (SELECT ... EXCEPT SELECT * FROM tgt)
-- [SEM-10] lines 437-441: gbp_to_usg_curr alias misleading but join correct — NOT renamed (per lessons learned)
-- ============================================================================

-- TODO(SME) [SEM-05]: Is wrk_dc_priority a SET table? If so, this INSERT must be
--   ported as INSERT INTO wrk_dc_priority ( SELECT ... EXCEPT SELECT * FROM wrk_dc_priority )
--   to preserve Teradata SET-table dedup semantics.
...
SELECT
    ...
    best_usg.opr_myflight_no               AS opr_myflight_no,
    best_usg.opr_myflight_sfx_cd           AS opr_myflight_sfx_cd,
    ...
    best_usg.alliance_cd              AS alliance_cd,
    ...
    best_usg.ex_to_sa_dt          AS ex_to_sa_dt,
    ...
    CASE
        WHEN (
            COALESCE(
                (
                    (CAST(COALESCE(best_sa.dc_no, 0) AS BIGINT)   -- TODO(SME) [DEF-04]: was CAST((best_sa.dc_no, 0, 9999999999) AS BIGINT); confirm constant 0 / dropped 9999999999
                     - CAST(COALESCE(best_sa.pme_dc_no, 0) AS BIGINT)  -- TODO(SME) [DEF-04]: was CAST((best_sa.pme_dc_no, 0, 9999999999) AS BIGINT); confirm constant 0 / dropped 9999999999
                    ) * 4
                ), 0
            ) + best_sa.cou_no
        ) BETWEEN 1 AND 16
...
END AS yq_vlu,
-- TODO(SME) [DEF-05]: yq_curr_vlu SELECT expression was missing (INSERT has yq_curr_vlu at line 94).
--   Synthesized as a parallel CASE mirroring the yq_vlu branches but returning the
--   original-currency value (no / ex_rte conversion) where yq_vlu returns the GBP value.
--   Confirm the exact business rule against the Teradata source.
CASE

    WHEN (best_sa.air_no = '000' AND best_usg.src_cd <> 'MY')
    THEN best_sa.yq_curr_vlu


    WHEN best_sa.air_no = '000'
    THEN MY_src.yq_curr_vlu
...
LEFT JOIN ref_ctry up_ctry_grp
      ...
      AND best_usg.up_dt BETWEEN up_ctry_grp.bus_efast_start_dt AND up_ctry_grp.bus_efast_end_dt     /* v1.26 TODO(SME) [SFX-01]: was PERIOD operator `bus_efast_pd` on `best_usg.up_dt`; confirm split column names */
...
LEFT JOIN ref_ctry disch_ctry_grp
      ...
      AND best_usg.up_dt BETWEEN disch_ctry_grp.bus_efast_start_dt AND disch_ctry_grp.bus_efast_end_dt      /* v1.26 TODO(SME) [SFX-01]: was PERIOD operator `bus_efast_pd` on `best_usg.up_dt`; confirm split column names */
