...
**Reviewer guidance:** `07_review/output/review_SFIssueSET.md` does **not**
exist on disk at this time. However, `02_rules/06_lessons_learned.md` records the
reviewer's authoritative confirmation verbatim: *"This is a SET table in
teradata. So, there should not be any duplicates. You need to handle the dedup
logic in the query."* → `staging.wrk_target` is a **SET table**; SEM-05 is
confirmed and classified **FIX** (not SME). Treat the lessons-learned record as
the authoritative reviewer guidance for this run.

**Prior-run status:** The SEM-05 SET-table fix (split the single
`INSERT … UNION … UNION` into 3 independent
`INSERT INTO staging.wrk_target (col list) (SELECT … EXCEPT SELECT * FROM
staging.wrk_target)`) was applied in prior runs (2026-07-24 / 2026-07-25) and
lives in archived `_fixed.sql` / `_final.sql` outputs. **The current input
file is still the original buggy single-`INSERT … UNION … UNION` form** — the
fix has not been merged back into `00_input/`. This analysis therefore audits
the buggy input as-is; Stage 3 will re-apply the SEM-05 fix.

---

## 1. Column-count check (INSERT target cols vs SELECT expressions)

The statement is a single `INSERT INTO staging.wrk_target` (line 1, no explicit
target column list) followed by a 3-branch `UNION` (lines 30, 60).
...
| # | SELECT expression | Alias | Lines (branch 1 / 2 / 3) |
|---|---|---|---|
| 1 | `src.airline_cd` | — | 3 / 33 / 63 |
| 2 | `src.flgt_no` | — | 4 / 34 / 64 |
| 3 | `src.flgt_dt` | — | 5 / 35 / 65 |
| 4 | `SUBSTR(MAX(src.effective_dt \|\| leg.company_cd), 11, 99)` | `company_cd` | 6 / 36 / 66 |
| 5 | `SUBSTR(MAX(src.effective_dt \|\| leg.direction_ind), 11, 99)` | `direction_ind` | 7 / 37 / 67 |
| 6 | `SUBSTR(MAX(src.effective_dt \|\| leg.operating_airline_cd), 11, 3)` | `operating_airline_cd` | 8 / 38 / 68 |
| 7 | `SUBSTR(MAX(src.effective_dt \|\| leg.operating_flgt_no), 11, 99)` | `operating_flgt_no` | 10 / 40 / 70 |
| 8 | `SUBSTR(MAX(src.effective_dt \|\| leg.operating_sfx_cd), 11, 99)` | `operating_sfx_cd` | 12 / 42 / 72 |
...
- **Signal:** one `INSERT INTO staging.wrk_target` (line 1) + 3 `UNION`
  branches (lines 30, 60), all writing to the same target.
- **DDL supplied?** No — but the **reviewer has confirmed** (per
  `06_lessons_learned.md`) the target is a SET table, which is the missing DDL
  fact SEM-05 requires. Per the Stage-1 contract, reviewer confirmation closes
  the SME question and moves SEM-05 to **FIX**.
...
pending source DDL / Teradata ground truth. The SEM-05 fix was applied in prior
runs and lives in archived `_fixed.sql` outputs; the input file itself remains
the original buggy form, so Stage 3 will re-apply the fix.